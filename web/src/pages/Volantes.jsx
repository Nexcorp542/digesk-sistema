...
function docToForm(d) {
  return {
    ...
    cliente: d.cliente || '',
    caras: d.caras || 'Una cara',
    millares: d.cantidad ?? 1,
    precioVenta: d.precioVenta ?? '',
    costo: d.cantidad ? round2((d.invTotal ?? 0) / d.cantidad) : '',
    abono: d.abono ?? '',
  };
}
...
export default function Volantes() {
  ...
  const loadDocs = () => apiGet('/api/volantes').then(setDocs);
  ...
  const invTotal = round2(millares * costo);
  const utilidad = round2(total - invTotal);
  const saldo = round2(total - abono);

  const buildPayload = () => ({
    fecha: form.fecha, cliente: form.cliente, caras: form.caras,
    cantidad: millares, precioVenta, invCantidad: millares, invTotal,
    total, utilidad, saldo, abono, inversion: invTotal,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    if (editingId) {
      await apiPatch(`/api/volantes/${editingId}`, buildPayload());
    } else {
      await apiPost('/api/volantes', buildPayload());
    }
    setSaving(false);
    cancelarEdicion();
    loadDocs();
  ...
  const handleMarkPaid = async (doc) => {
    await apiPatch(`/api/volantes/${doc.id}`, {
  ...
  const handleDelete = async (id) => {
    ...
    await apiDelete(`/api/volantes/${id}`);
  ...
  return (
    <div className="p-6">
      <h1 className="text-white text-xl font-semibold mb-6">Volantes</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6 items-start">
        <form onSubmit={handleSubmit} className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5 space-y-4 lg:sticky lg:top-6">
          ...
          <Field label="Impresión">
            <select value={form.caras} onChange={(e) => handleChange('caras', e.target.value)} className={inputClass}>
        ...
        <RecordList
          ...
          titleFn={(d) => `Volantes${d.cliente ? ` · ${d.cliente}` : ''} (${d.caras || ''})`}
          subtitleFn={(d) => `${d.cantidad} millar${d.cantidad === 1 ? '' : 'es'} · Ver detalles`}
          onEdit={handleEdit}
