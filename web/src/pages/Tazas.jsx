...
export default function Tazas() {
  ...
  const loadDocs = () => apiGet('/api/tazas').then(setDocs);
  ...
  const buildPayload = () => ({
    fecha: form.fecha, cliente: form.cliente, cantidad,
    precioUnit: precioVenta, inversion, total, utilidad, saldo, abono,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    if (editingId) {
      await apiPatch(`/api/tazas/${editingId}`, buildPayload());
    } else {
      await apiPost('/api/tazas', buildPayload());
    }
    setSaving(false);
    cancelarEdicion();
    loadDocs();
  ...
  const handleMarkPaid = async (doc) => {
    await apiPatch(`/api/tazas/${doc.id}`, {
  ...
  return (
    <div className="p-6">
      <h1 className="text-white text-xl font-semibold mb-6">Tazas</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6 items-start">
        ...
        <RecordList
          ...
          titleFn={(d) => `Taza${d.cliente ? ` · ${d.cliente}` : ''}`}
